    ...
    def batchnorm_modify(self):
        for layer, (name, module) in enumerate(self.model.features._modules.items()):
            if layer < 4 or layer > 7:
                ...
                for kt in range(2):
                    conv1 = torch.nn.BatchNorm2d(model.features._modules.items()[layer][1][kt].conv1.out_channels,
                                                 eps=1e-05, momentum=0.1, affine=True)
                    model.features._modules.items()[layer][1][kt].bn1 = conv1
                    conv2 = torch.nn.BatchNorm2d(model.features._modules.items()[layer][1][kt].conv2.out_channels,
                                                 eps=1e-05, momentum=0.1, affine=True)
                    model.features._modules.items()[layer][1][kt].bn2 = conv2
                    if layer > 4 and layer < 8 and kt == 0:
                        convd = torch.nn.BatchNorm2d(model.features._modules.items()[layer][1][kt].conv2.out_channels,
                                                     eps=1e-05, momentum=0.1, affine=True)
                        ds = torch.nn.Sequential(
                            *(replace_layers(model.features._modules.items()[layer][1][kt].downsample, i, [1], \
                                             [convd]) for i, _ in
                              enumerate(model.features._modules.items()[layer][1][kt].downsample)))
